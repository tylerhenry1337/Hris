...
public class Benefits {
    ...
    static void updateBenefits(int benId,int empId,String savings, String retirement, double pto,double sickDays,double pDays, String cPerks){
        try {
            Connection con = DriverManager.getConnection(url,user,pw);
            Statement st = con.createStatement();
            //UPDATE
            boolean update = st.execute("UPDATE benefits\n" +
                    "SET\n" +
                    "savings = '"+savings+"',\n" +
